...
CREATE TABLE IF NOT EXISTS timetrack.absences (
                                    ...
                                    absence_type_id UUID NOT NULL REFERENCES timetrack.absence_types(id) ON DELETE RESTRICT,
    ...
    -- Fields from ApprovableEntity (assumed based on parent class)
                                    approved_by UUID REFERENCES personnel.employees(id) ON DELETE SET NULL,
...
-- Add check constraint for time logic (if both times are provided)
ALTER TABLE timetrack.absences
