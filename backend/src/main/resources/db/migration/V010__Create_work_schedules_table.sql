...
CREATE INDEX IF NOT EXISTS idx_work_schedules_effective_until ON timetrack.work_schedules(effective_until);
CREATE INDEX IF NOT EXISTS idx_work_schedules_is_active ON timetrack.work_schedules(is_active);
CREATE INDEX IF NOT EXISTS idx_work_schedules_is_working_day ON timetrack.work_schedules(is_working_day);
CREATE INDEX IF NOT EXISTS idx_work_schedules_created_at ON timetrack.work_schedules(created_at);

-- Composite indexes for common queries
CREATE INDEX IF NOT EXISTS idx_work_schedules_employee_effective ON timetrack.work_schedules(employee_id, effective_from, effective_until);
CREATE INDEX IF NOT EXISTS idx_work_schedules_active_dates ON timetrack.work_schedules(is_active, effective_from, effective_until);

-- Create trigger to automatically update updated_at
CREATE TRIGGER update_work_schedules_updated_at
    BEFORE UPDATE ON timetrack.work_schedules
    FOR EACH ROW
...
INSERT INTO timetrack.work_schedules (employee_id, day_of_week, start_time, end_time, break_duration_minutes, is_working_day, effective_from, effective_until, is_active)
SELECT
    e.id,
...
WHERE e.is_active = true
  AND NOT EXISTS (
    SELECT 1 FROM timetrack.work_schedules ws
...
COMMENT ON TABLE timetrack.work_schedules IS 'Employee regular working hours and schedules';
COMMENT ON COLUMN timetrack.work_schedules.id IS 'Unique identifier for the work schedule';
COMMENT ON COLUMN timetrack.work_schedules.employee_id IS 'Reference to the employee this schedule belongs to';
COMMENT ON COLUMN timetrack.work_schedules.day_of_week IS 'Day of week (1=Monday, 2=Tuesday, ..., 7=Sunday)';
COMMENT ON COLUMN timetrack.work_schedules.start_time IS 'Start time for this day (null if non-working day)';
COMMENT ON COLUMN timetrack.work_schedules.end_time IS 'End time for this day (null if non-working day)';
COMMENT ON COLUMN timetrack.work_schedules.break_duration_minutes IS 'Total break time in minutes for this day';
COMMENT ON COLUMN timetrack.work_schedules.is_working_day IS 'Whether this is a working day for the employee';
COMMENT ON COLUMN timetrack.work_schedules.effective_from IS 'Date when this schedule becomes effective';
COMMENT ON COLUMN timetrack.work_schedules.effective_until IS 'Date when this schedule expires';
